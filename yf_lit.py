...
if side_bar == 'Home':
    ...
    if st.button('Run Analysis'):
        ...
        especially the news, and provide a recommendation for buying, holding, and selling.
        '''
        ...
        def fetch_llm_response():
            """Wrapper to consume async generator in a sync function."""
            queue = asyncio.Queue()

            async def _fetch():
                async for text in y.llm(prompt=prompt):
                    await queue.put(text)  # Put text into queue
                await queue.put(None)  # Signal end of stream

            async def _stream():
                task = asyncio.create_task(_fetch())  # Run `_fetch()` in background
                while True:
                    text = await queue.get()  # Get text from queue
                    if text is None:
                        break
                    yield text

            return _stream()  # ✅ Returns a generator that works with `write_stream()`

        # ✅ Stream response in sidebar
        st.sidebar.write('Investment Report (Generated by AI)')
        st.sidebar.write_stream(fetch_llm_response())  # ✅ No need for async
